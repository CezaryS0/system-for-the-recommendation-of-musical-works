...
class Autoencoder1D:
    ...
    def autoencode(self,shape):
        ...
        encoded = Conv1D(filters=8,kernel_size=3, activation='relu',padding='same')(encoded)
        encoded = MaxPooling1D(2, padding='same')(encoded)
        encoded = Conv1D(filters=4,kernel_size=3, activation='relu',padding='same')(encoded)
        encoded = MaxPooling1D(2, padding='same')(encoded)
        encoder = Model(input_img,encoded)
        
        decoded = Conv1D(filters=4,kernel_size=3, activation='relu',padding='same')(encoded)
